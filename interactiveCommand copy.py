...
class CableRobotGUI:
    def __init__(self, root):
        ...
        self.ani = animation.FuncAnimation(self.fig, self.update_plot, interval=100, blit=False, cache_frame_data=False, save_count=100)
    ...
    def on_closing(self):
        ...
        # Close motor connection if open
        if self.motor:
            try:
                self.motor.close()
            except:
    ...
    def animate_visualization(self):
        if self.is_visualizing and self.viz_index < len(self.viz_points) and self.root.winfo_exists():
    ...
    def process_queue(self):
        ...
        # Only schedule the next call if the window still exists
